...
import com.ryan.bilibili_client.module.user.UserInfoDetailsActivity;
...
import com.ryan.bilibili_client.widget.livelike.LoveLikeLayout;
        ...
        RetrofitHelper.getLiveAPI()
                ...
                .map(responseBody -> {
                    try {
                        ...
                        result = result.substring(result.lastIndexOf("[") + 1, result.lastIndexOf("]") - 1);
                        return result;
                    } catch (IOException e) {
                        e.printStackTrace();
                        return null;
